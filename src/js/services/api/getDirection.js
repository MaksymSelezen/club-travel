    ...
    data.data.forEach(hotel => {
      ...
      hotel.country.regions.forEach(region => {
        ...
        uniqueRegions.push({
          ...
          offersCount: 1,
          dates: [
            formatDate(hotel.dateOfDeparture),
...
export const getRegions = async () => {
  try {
    const res = await fetch(`${API_URL}v2-regions?fields=name,slug&pagination[pageSize]=100`);
    const data = await res.json();
    return data.data;
  }
  catch (error) {    
...
export const getCountries = async () => {
  try {
    const res = await fetch(`${API_URL}v2-countries?fields=name&pagination[pageSize]=100`);
    const data = await res.json();
    return data.data;
  }
  catch (error) {    
